...
public class CustomRpcSender
{
    ...
    public State CurrentState
    {
        ...
        set
        {
            ...
            else VentLogger.Warn("CurrentStateはisUnsafeがtrueの時のみ上書きできます", "CustomRpcSender");
        }
    }
    ...
    public CustomRpcSender(string name, SendOption sendOption, bool isUnsafe)
    {
        ...
        onSendDelegate = () => VentLogger.Old($"{this.name}'s onSendDelegate =>", "CustomRpcSender");

        currentState = State.Ready;
        VentLogger.Old($"\"{name}\" is ready", "CustomRpcSender");
    }
    ...
    public CustomRpcSender StartMessage(int targetClientId = -1)
    {
        if (currentState != State.Ready)
        {
            ...
            if (isUnsafe)
            {
                VentLogger.Warn(errorMsg, "CustomRpcSender.Warn");
            }
            else
            {
    ...
    public void SendMessage()
    {
        ...
        VentLogger.Old($"\"{name}\" is finished", "CustomRpcSender");
        stream.Recycle();
    }
